...
app v x = VApp v x            -- cannot reduce, store it as value
...
-- the conversion algorithm; the integer is used to represent the
-- introduction of a fresh variable VGen == gensym. k is not used as a
-- state, because the check is recursive.
